          ...
          hover:bg-[#0F3A2B] transition"
              ...
              <button
                onClick={() => {
                  ...
                  // 1️⃣ Create a fresh syllabus tree
                  const resetTree = normalizeTree(TREE);

                  // 2️⃣ Combined updates to push to backend + localStorage
                  const updates = {
                  ...
                  // 3️⃣ Update global dashboard state (App.jsx handles caching + backend)
                  updateDashboard(updates);

                  // 4️⃣ Update UI instantly
                  treeRef.current = resetTree;
                  forceRender((x) => x + 1); // ensures instant visual refresh
                }}
                className="
